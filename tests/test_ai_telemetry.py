...
@pytest.mark.parametrize(
    ("pricing", "message"),
    [
        ({"gpt-test": AIModelPricing(input_cost_per_token=-1)}, "pricing"),
    ],
)
def test_rejects_invalid_pricing(
    pricing: dict[str, AIModelPricing], message: str
) -> None:
    with pytest.raises(AIConfigurationError, match=message):
        AIUsageTelemetry(pricing)
